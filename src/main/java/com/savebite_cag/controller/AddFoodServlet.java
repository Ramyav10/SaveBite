...
public class AddFoodServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        boolean status = dao.addFoodItem(food);

        if (status) {
            response.sendRedirect(request.getContextPath() +
                "/RestaurantDashboardServlet?tab=menu&success=Item added!");
        } else {
            request.setAttribute("error", "Failed to add item.");
            request.getRequestDispatcher("/jsp/addFoodItem.jsp").forward(request, response);
        }
    }
